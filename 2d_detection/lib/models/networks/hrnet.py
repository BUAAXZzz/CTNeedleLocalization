...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.inplanes = last_inp_channels
        ...
        self.deconv_layers = nn.Sequential(
            nn.Conv2d(last_inp_channels, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        )
...
def _hrnet(arch, pretrained, progress, heads, head_conv, **kwargs):
    model = HighResolutionNet(MODEL_CONFIGS[arch], heads, head_conv, **kwargs)
    if pretrained:
        ...
        # 定义本地路径
        model_path = '/hdd2/xz/models/hrnetv2_w48_imagenet_pretrained.pth'
        
        try:
          state_dict = torch.load(model_path)
          model.load_state_dict(state_dict, strict=False)
          print(f"Successfully loaded pre-trained weights from local path: '{model_path}'")
        except FileNotFoundError:
          print(f"ERROR: Pre-trained weights file not found at '{model_path}'. Model initialization failed.")
        except Exception as e:
          print(f"ERROR: An exception occurred while loading pre-trained weights: {e}")
    return model


def hrnet18(heads, head_conv, **kwargs):
    r"""HRNet-18 model
    """
    return _hrnet('hrnet18', True, True, heads, head_conv,
                   **kwargs)


def hrnet32(heads, head_conv, **kwargs):
    r"""HRNet-32 model
    """
    return _hrnet('hrnet32', True, True, heads, head_conv,
                   **kwargs)


def hrnet48(heads, head_conv, **kwargs):
    r"""HRNet-48 model
    """
    return _hrnet('hrnet48', True, True, heads, head_conv,
                   **kwargs)


def get_hrnet48(num_layers, heads, head_conv):
  model = hrnet48(heads, head_conv)
  return model


def get_hrnet32(num_layers, heads, head_conv):
  model = hrnet32(heads, head_conv)
  return model
...
#     # -----------------------------
#     # 1) 构造一个最小 heads，保证 forward 能走通
#     #    你也可以按你工程里的 heads 改，比如 {'hm':2,'wh':2,'reg':2} 等
#     # -----------------------------
#     heads = {
#         "hm": 2,     # 举例：2类热图
#         "reg": 2     # 举例：偏移
...
#     # -----------------------------
#     # 2) 构造模型：为了调试尺寸，建议先关闭 pretrained，避免硬编码路径报错
#     # -----------------------------
#     model = _hrnet("hrnet48", pretrained=False, progress=False, heads=heads, head_conv=head_conv)
#     model.eval()

#     # -----------------------------
#     # 3) 注册 hook：打印每个模块的输入/输出 shape
#     #    对于输出是 list/tuple/dict 的情况也做展开打印
#     # -----------------------------
#     def _shape_str(x):
#         if torch.is_tensor(x):
#             return f"{list(x.shape)}"
#         if isinstance(x, (list, tuple)):
#             return "[" + ", ".join(_shape_str(t) for t in x) + "]"
...
#             in_str = _shape_str(inp[0]) if isinstance(inp, (tuple, list)) and len(inp) > 0 else _shape_str(inp)
#             out_str = _shape_str(out)
#             print(f"[{name:<35}]  in: {in_str:<25} -> out: {out_str}")
#         return hook

#     # 只 hook 你最关心的关键节点，避免输出刷屏
#     hook_names = [
#         "conv1", "conv2", "layer1",
#         "transition1", "stage2",
#         "transition2", "stage3",
...
#     # 另外把每个 head 也 hook 一下，方便看到最终输出通道数
#     for head_name in heads.keys():
#         if hasattr(model, head_name):
#             hooks.append(getattr(model, head_name).register_forward_hook(make_hook(f"head:{head_name}")))
