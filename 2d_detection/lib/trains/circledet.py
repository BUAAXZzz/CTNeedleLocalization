...
class CircleTrainer(BaseTrainer):
  ...
  def debug(self, batch, output, iter_id):
    ...
    for i in range(1):
      ...
      # SAVE
      # save_dir = '/hdd/xz/CircleNet/vis/'
      # os.makedirs(save_dir, exist_ok=True)
      # cv2.imwrite(os.path.join(save_dir, 'img.png'), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
      # cv2.imwrite(os.path.join(save_dir, 'pred.png'), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
      # cv2.imwrite(os.path.join(save_dir, 'gt.png'), cv2.cvtColor(gt, cv2.COLOR_RGB2BGR))
  ...
  def save_result(self, output, batch, results):
    reg = output['reg'] if self.opt.reg_offset else None     # self.opt.reg_offset = True    reg: [1, 2, 128, 128]
    dets = circledet_decode(
      output['hm'], output['cl'], reg=reg,
      cat_spec_wh=self.opt.cat_spec_wh, K=self.opt.K)        # dets: [1, 1000, 5]
